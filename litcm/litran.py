...
class LIT():
    ...
    def __init__(self, labels=['hin', 'eng'], transliteration=False):
        ...
        for tag in self.labels:
            self.blm_wp.append(kenlm.LanguageModel(
                '{}/blm_models/{}.tk.blm'.format(path, tag)))
            self.blm_sp.append(kenlm.LanguageModel(
                '{}/blm_models/{}.ts.blm'.format(path, tag)))
    ...
    def mapper(self, word, ip_tag, op_tag):
        ...
        map_string = [chr(ord(letter)-bases[op_tag] + bases[ip_tag])
                      if ord(letter) in range(3328, 3456) else letter
                      for letter in list(word.decode("utf-8"))]

        return "".join([ch for ch in map_string if ud.category(chr(ord(ch))) != "Cn"]).encode("utf-8")

    def transliterate(self, word, tag):
        """Transliterate words predicted as Indic-words to their native scripts"""

        # transliterate to WX
        wx = tn.RomanConvertor(
            self.tree[self.tag_dct[tag]], word, tag).transliterate()
        ...
        if tag == "kan":
            ...
            self.idfsen += "%s\\Kan=%s " % (word, self.mapper(
                unicodeString, "kannada", "malayalam"))
            return
